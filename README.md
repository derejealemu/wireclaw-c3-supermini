...
**ESP32-C3 Super Mini note:** Limited SRAM (~400KB, no PSRAM). Requires a Telegram proxy
on your LAN. See [docs/C3-MINI.md](docs/C3-MINI.md) for details and all
...
- **Supported:** ESP32-C6, ESP32-S3, ESP32-C3 (4MB flash)
- **Platform:** [pioarduino](https://github.com/pioarduino/platform-espressif32) via PlatformIO
- **ESP32-C3 Super Mini:** ~400KB SRAM, no PSRAM — requires Telegram proxy (see [docs/C3-MINI.md](docs/C3-MINI.md))
...
### ESP32-C3 Super Mini Users: Telegram Proxy Required

The ESP32-C3 (~400KB SRAM) cannot sustain TLS to `api.telegram.org` alongside
HTTP LLM calls. WireClaw on C3 requires a **plain-HTTP Telegram proxy** on your
LAN server. See [docs/C3-MINI.md](docs/C3-MINI.md) for setup.
...
The C3 firmware is pre-configured to send Telegram requests to the proxy.
Just build and flash normally (see below).
...
| [ESP32-C3 Super Mini](docs/C3-MINI.md) | ESP32-C3 optimizations, Telegram proxy, memory tuning |
