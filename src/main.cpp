...
bool connectWiFi() {
    ...
    WiFi.mode(WIFI_STA);
    // Explicit DNS to avoid DHCP DNS failures (Google + Cloudflare)
...
/* error reporting now handled inside chatWithLLM() returning the string directly */
...
const char *chatWithLLM(const char *userMessage) {
    ...
    if (ok && finalContent && finalContent[0]) {
        ...
        static char finalError[256];
        ledRed();
        const char *lerr = llm.lastError();
        Serial.printf("\n[ERROR] LLM call failed: %s\n\n", lerr ? lerr : "(null)");
        Serial.flush();
        if (lerr && lerr[0]) {
            snprintf(finalError, sizeof(finalError), "[LLM] %s", lerr);
        } else {
            snprintf(finalError, sizeof(finalError), "[LLM] unknown");
        }
        chatActive = false;
        return finalError;
    }
...
static WiFiClient tgClient;   /* HTTP-only: uses daiserver proxy instead of direct TLS */
...
/* Telegram proxy on LAN server (C3 cannot do TLS to api.telegram.org directly).
 * Change TG_HOST/TG_PORT to match your proxy server before building.
 * See docs/C3-MINI.md and scripts/telegram-proxy.py for setup. */
static const char *TG_HOST = "192.168.0.55";
static const int   TG_PORT = 9443;
...
static void telegramTick() {
    ...
    case TG_WAITING: {
        ...
        tgSendMessage(response ? response : "[LLM] no response text");
...
static void tgYield() {
    if (tgState != TG_IDLE) {
        ...
        tgState = TG_IDLE;
        tgLastPoll = millis();
        delay(3000); /* Let LWIP TCP TIME_WAIT release before next connection */
    }
}
...
void setup() {
    ...
    if (cfg_telegram_token[0] != '\0' && cfg_telegram_chat_id[0] != '\0') {
        ...
        /* tgClient is WiFiClient (no SSL) — proxy handles TLS on daiserver */
